...
message.innerHTML = '<h1>Chat PST</h1><br>..Hi I am a Interactive platform for VLSI Community<br>..I am at my infant stage<br>..made and maintained by Team Silicon Symphony<br>..I have miles to go before I sleep';
...
function sendQuery() {
    ...
    // const url='/api/v1.0/get-suggested-ques/'
    // const suggestedQuestions1= fetch(url)
    // const suggestedQuestions = suggestedQuestions1.slice(0, 4);
    function fetch_suggestion(){
    const url = '/api/v1.0/get-suggested-ques/';
    fetch(url)
    .then(response => response.json()) // Extract the data from the response
    .then(data => {
        const suggestedQuestions= data.slice(0, 4); // Extract the first four suggested questions
        console.log(suggestedQuestions)
         // Do something with the suggested questions, e.g. render them on the page
        suggestedQuestionsDiv = document.querySelector(".suggestedQuestions");
        suggestedQuestionsDiv.innerHTML = "";
    ...
    // [
    //     "What is Immediate Assertion?",
    //     "What is Severity?",
    //     "What is Concurrent Assertion?",
    //     "Code for Assertion?",
    // ];
    //= `/api/v1.0/get-suggested-ques/`;
